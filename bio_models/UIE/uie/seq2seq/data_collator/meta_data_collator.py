...
class DynamicSSIGenerator():
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase, schema: RecordSchema, positive_rate=1, negative=5, ordered_prompt=False) -> None:
        self.spot_dict = self.get_ordered_dict(schema.type_list, tokenizer) # 顺序字典 key是分词化的token
        ...
        self.spot_prompt = tokenizer.get_vocab()[spot_prompt] # <spot>的词表
        self.asoc_prompt = tokenizer.get_vocab()[asoc_prompt] # <asoc>的词表
        self.text_start = tokenizer.get_vocab()[text_start] # <text>的词表
    ...
    def __call__(self, features):
        ...
        for feature in features:
            ...
            if not sample_prompt:
                ...
                converted_spot_prefix, positive_spot, negative_spot = self.negative_sampler.sample_spot(feature.get('spots', [])) # feature.get('spots', [])
            ...
            # truncate `prefix` to max length 截断前缀
            ...
            # truncate `input_ids` to max length 截断输入和输出
            if self.max_length:
            ...
            feature['attention_mask'] = [1] * len(feature['input_ids']) # 获取attention_mask
