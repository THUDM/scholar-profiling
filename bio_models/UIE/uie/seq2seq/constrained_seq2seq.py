...
class ConstraintSeq2SeqTrainer(Seq2SeqTrainer):
    def __init__(self, decoding_type_schema=None, task='event', decoding_format='tree', source_prefix=None, *args, **kwargs):
        ...
        if self.args.constraint_decoding:
            # SpotConstraintDecoder or SpotAsocConstraintDecoder
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch):
        if self.control.should_log:
            ...
            logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4) # 保留四位小数
        ...
        if self.control.should_save:
            self._save_checkpoint(model, trial, metrics=metrics) # 保存模型 更新最优指标
    ...
    def prediction_step(
        ...
        # in case the batch is shorter than max length, the output should be padded 填充生成输出
