...
class Text2SpotAsoc(GenerationFormat):
    ...
    def annonote_graph(self,
        ...
        def add_spot(spot):
            ...
            spot_dict[spot_key] = spot # {((1,), label):entity}

            if self.get_label_str(spot.label) not in self.record_role_map:
                self.record_role_map[self.get_label_str(spot.label)] = set() # 为每个role构建集合存放asoc_role

        def add_asoc(spot, asoc: Label, tail):
            spot_key = (tuple(spot.span.indexes), self.get_label_str(spot.label)) # 头实体：((1,), label)
            asoc_dict[spot_key] += [(tail.span.indexes, tail, self.get_label_str(asoc))] # 尾实体：((1,), tail,label)

            self.record_role_map[self.get_label_str(spot.label)].add(self.get_label_str(asoc)) # 添加映射

        # 获取spot asoc字典 并将spot:asoc映射存入self.record_role_map
        for entity in entities:
        ...
        for spot_key in sorted(spot_dict.keys()):
            ...
            for _, tail, asoc in sorted(asoc_dict.get(spot_key, [])):
                ...
                spot_instance['asoc'] += [(asoc, tail.span.text)] # (tail标签，tail文本)
