...
def convert_graph(
    ...
    os.makedirs(output_folder, exist_ok=True) # 输出路径
    ...
    for data_type, instance_list in datasets.items():
        with open(os.path.join(output_folder, f"{data_type}.json"), "w") as output:
            for instance in tqdm(instance_list):
                ...
                src, tgt, spot_labels, asoc_labels = converted_graph[:4] # 输入文本 输出文本 spot_labels asoc_labels
                spot_asoc = converted_graph[4] # spot：asoc映射 包含标签和文本
                
                # 存放实体 关系 事件 用于导出schema
...
def main():
    ...
    generation_class = generation_format_dict.get(options.generation_format) # Text2SpotAso类
    # 获取要生成的文件
    ...
    for filename in config_list:
        dataset = Dataset.load_yaml_file(filename) # 设置数据集的相关配置

        datasets = dataset.load_dataset() # 加载训练集 验证集 测试集的数据
