...
class EntDataset(Dataset):
    ...
    def get_new_ins(self, bpes, spans, indexes):
            if len(bpes)<self.max_len:
                ...
                indexes.append(0)
                # int8范围-128~127
                matrix = np.zeros((len(ent2id), cur_word_idx, cur_word_idx), dtype=np.int8)
                ent_target = []
                for _ner in spans:
                    s, e, t = _ner
                    matrix[t, s, e] = 1
                    ent_target.append((t, s, e))
        ...
        for item in data:
            ...
            for _raw_words, _raw_ents in zip(raw_sents, raw_entss):
                ...
                if len(bpes) + len(_bpes) <= self.max_len:
                    ...
                    new_ins = self.get_new_ins(bpes, spans, indexes) # 测试集会存在额外ins 不影响结果
    ...
    def collate(self, examples):
        ...
        batch_labels = torch.tensor(self.sequence_padding(batch_matrix, seq_dims=3)).long()
