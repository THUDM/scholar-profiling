...
class EntDataset(Dataset):
    ...
    def collate(self, examples):
        ...
        for item in examples:
            ...
            labels = np.zeros((len(ent2id), max_len, max_len)) # 实体矩阵
            spoes = set()
            en = {}
            for entity, label in item[1:]: # start, end
                if entity not in en:
                    en[entity] = 1
                else: en[entity] +=1
                o = self.tokenizer.encode(entity, add_special_tokens=False)
                oh = search(o, input_ids, en[entity]) # 仅标记最大长度以内的实体
                # oh = search(o, input_ids)
                if oh != -1:
                    labels[label, oh, oh+len(o)-1] = 1
                oh = search(o, text_ids, en[entity]) # 统计所有实体
                # oh = search(o, text_ids)
                if oh != -1:
                    spoes.add((label, oh, oh+len(o)-1))
