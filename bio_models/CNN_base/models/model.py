...
class CNNNer(nn.Module):
    ...
    # matrix -> (b, s, s, l)
    def forward(self, input_ids, attention_mask, token_type_ids, matrix=None):
        # bert
        outputs = self.pretrain_model(input_ids, attention_mask, token_type_ids, return_dict=True)
        state = outputs['last_hidden_state']
        # 构建双仿射矩阵
        head_state = self.head_mlp(state)
        tail_state = self.tail_mlp(state)
        if hasattr(self, 'U'): # (bs, r, n, n)
            scores1 = torch.einsum('bxi, oij, byj -> boxy', head_state, self.U, tail_state) # 不使用多头
        else:
        ...
        if hasattr(self, 'cnn'):
            mask = attention_mask
            mask = mask[:, None] * mask.unsqueeze(-1) # (b, 1, s) * (b, s,1)
